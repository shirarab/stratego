...
def max_my_soldier_degree_heuristic(game_state: GameState, color: Color):
    val = 0
    for s in game_state.soldier_knowledge_base[color]:
...
def opp_distance_to_flag_heuristic(game_state: GameState, color: Color):
    op_color = Color.RED if color == Color.BLUE else Color.BLUE
    loc = []
    for s in game_state.soldier_knowledge_base[op_color]:
        loc.append([s.x, s.y])
    my_flag = [s for s in game_state.soldier_knowledge_base[color] if s.degree == Degree.FLAG]
    if len(my_flag) == 0:
        return -10
    my_flag = my_flag[0]
